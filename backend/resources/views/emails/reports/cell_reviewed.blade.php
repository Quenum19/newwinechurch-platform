...
@php
  ...
  $headline = $isApproved ? 'Rapport cellule approuvé !'
            : ($isRejected ? 'Rapport cellule à corriger' : 'Rapport cellule revu');
@endphp
...
@if($isApproved)
  <p>Félicitations <strong style="color:#C9A84C;">{{ $recipient->first_name ?? $recipient->name }}</strong> !
    Le rapport de la cellule <strong>{{ $cell?->name }}</strong> a été
    <strong style="color:#1F8B3D;">approuvé</strong>
  ...
  <p style="color:rgba(255,255,255,0.85);">Continue ton service pour la gloire de Dieu 🙌</p>
@elseif($isRejected)
  <p>Le rapport soumis pour la cellule <strong>{{ $cell?->name }}</strong> nécessite des
  ...
  @endif
@else
  <p>Le rapport de la cellule <strong>{{ $cell?->name }}</strong> a été <strong>revu</strong>.</p>
  @if($report->review_comment)
